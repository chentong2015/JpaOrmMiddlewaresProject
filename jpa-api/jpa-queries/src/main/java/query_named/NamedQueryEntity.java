...
// TODO. 在entity class类型上声明query查询语句，通过name名称来直接调用
@Entity
...
// 这里具名Native查询语句必须声明返回的结果类型
@NamedNativeQuery(name = "EntityNamedNative.findAll", query = "SELECT * FROM t_named_query_table", resultClass = NamedQueryEntity.class)
// 可以设定一组集合的具名查询
// @NamedQueries({@NamedQuery(name = "EntityNamed.findAll", query = "xxx"), @NamedQuery(name = "EntityNamed.findByUrl", query = "xxx")})
